...
import { buildAccountSummary, prevMonthKey, type AccountSummary, type AccountTx } from "./account-core";
...
const mesLabel = (monthKey: string) => {
...
export type CardLens =
  | { temCartao: false }
  | {
      temCartao: true;
      vencendoMes: number; qtdVencendo: number;
      vencimentoMaisProximo: string | null;
      emAberto: number; algumaNaoConferida: boolean;
    };

export type DiagContext = {
  monthKey: string;
  summary: AccountSummary;
  byCategoryCodes: Array<[string, number]>; // CÓDIGOS (a home precisa pra cor/ícone)
  prevGasto: number;
  card: CardLens;
  stamp: number | null;
  stampLabel: string;
  ...
  // Empty states
  temMovimentoConta: boolean;   // alguma transação de conta no mês
  temDadoAlgum: boolean;        // conta OU cartão
...
export async function loadDiagContext(
  db: Firestore, workspaceId: string, monthKey: string
): Promise<DiagContext> {
  ...
  const toTx = (d: adminType.firestore.QueryDocumentSnapshot): AccountTx => {
    const x = d.data();
    return {
  ...
  const summary = buildAccountSummary({ transactions, prevTransactions, userNames });
  ...
  for (const t of accountExpenses) codeMap[t.category] = (codeMap[t.category] ?? 0) + t.amount;
  ...
  let card: CardLens = { temCartao: false };
  if (!faturasSnap.empty) {
    let vencendoMes = 0, emAberto = 0, qtdVencendo = 0;
    let vencimentoMaisProximo: string | null = null;
    let algumaNaoConferida = false;
    for (const d of faturasSnap.docs) {
      const f = d.data() as { totalAPagar?: number; vencimento?: string; verification?: string };
      const total = Number(f.totalAPagar ?? 0) || 0;
      emAberto += total;
      if (f.verification === "nao_conferido") algumaNaoConferida = true;
...
function dadosBlock(ctx: DiagContext): string {
  const s = ctx.summary;
  const r = s.renda;
...
function regrasBlock(ctx: DiagContext): string {
...
function guardrails(ctx: DiagContext): string {
...
async function askModel(client: Anthropic, prompt: string, maxTokens: number): Promise<Record<string, unknown>> {
  ...
  if (!m) throw new Error("diagnóstico veio fora do schema");
...
// ─────────────────────────────────────────────────────────────────────────────
export async function generateWhatsappDiagnosis(
  db: Firestore, client: Anthropic, workspaceId: string, monthKey: string
): Promise<{ texto: string; cached: boolean }> {
  const cacheRef = db.doc(`whatsappDiagnoses/${workspaceId}`);
  const ctx = await loadDiagContext(db, workspaceId, monthKey);

  const cached = await cacheRef.get();
  if (cached.exists) {
    const c = cached.data() as { version?: string; monthKey?: string; stamp?: number | null; texto?: string };
...
{ "texto": "a mensagem completa do WhatsApp, com quebras de linha \\n" }`;
  ...
  if (!texto) throw new Error("diagnóstico do WhatsApp veio vazio");
...
// ─────────────────────────────────────────────────────────────────────────────
// CANAL 2 — Home (narrativa + bullets + label). MESMO motor, MESMA verdade.
// ─────────────────────────────────────────────────────────────────────────────
export type HomeDiagnosisResult = {
  summary: AccountSummary;
...
export async function generateHomeDiagnosis(
  db: Firestore, client: Anthropic, workspaceId: string, monthKey: string
): Promise<HomeDiagnosisResult> {
  ...
  const cached = await cacheRef.get();
  if (cached.exists) {
    const c = cached.data() as { version?: string; monthKey?: string; stamp?: number | null; diag?: HomeDiag };
  ...
  const salvarEDevolver = async (diag: HomeDiag): Promise<HomeDiagnosisResult> => {
...
Retorne APENAS um JSON válido, sem markdown:
{
  ...
  const diag: HomeDiag = {
    narrativa: (out.narrativa as string) ?? null,
    bullet1: (out.bullet1 as string) ?? null,
    bullet2: (out.bullet2 as string) ?? null,
    scoreLabel: (out.scoreLabel as string) ?? null,
  };
  if (!diag.narrativa) throw new Error("diagnóstico da home veio vazio");
