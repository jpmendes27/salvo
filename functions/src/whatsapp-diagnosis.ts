...
import { buildAccountSummary, prevMonthKey, type AccountTx } from "./account-core";
...
async function loadAccountLens(db: Firestore, workspaceId: string, monthKey: string) {
...
async function loadCardLens(db: Firestore, workspaceId: string, monthKey: string) {
  const snap = await db.collection(`workspaces/${workspaceId}/faturas`).get();
  if (snap.empty) return { temCartao: false as const };
  ...
  for (const d of snap.docs) {
    const f = d.data() as {
      totalAPagar?: number; vencimento?: string; verification?: string;
    };
...
function buildPrompt(args: {
  monthKey: string;
  account: Awaited<ReturnType<typeof loadAccountLens>>;
  card: Awaited<ReturnType<typeof loadCardLens>>;
  dataStampLabel: string;
}): string {
...
export async function generateWhatsappDiagnosis(
  ...
  if (cached.exists) {
    const c = cached.data() as { monthKey?: string; stamp?: number | null; texto?: string; version?: string };
  ...
  if (!account.hasData && !card.temCartao) {
    const texto =
      "Ainda não tenho dado nenhum pra olhar aqui. 🤷\n\n" +
  ...
  const texto = (m ? (JSON.parse(m[0]) as { texto?: string }).texto : null) ?? "";
