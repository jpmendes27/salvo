...
exports.classifyIncome = classifyIncome;
...
// `internal` já é usado.
const internal_transfer_1 = require("./internal-transfer");
...
function nameSimilarity(sender, user) {
...
function isSameHolder(sender, userNames) {
...
function extractSenderName(description) {
    ...
    if (segments.length >= 2) {
        ...
        const isDoc = (s) => /[\d*•]/.test(s) && /^[\d*•.\-\/\s]+$/.test(s);
        const docIdx = segments.findIndex(isDoc);
        if (docIdx >= 1)
            return cleanNameTokens(segments[docIdx - 1]);
    }
    return cleanNameTokens(raw.replace(/\bPIX\b|\bTED\b|\bDOC\b|\bRECEBIDA?O?\b|\bENVIADA?O?\b|\bTRANSFERENCIA\b|\bDEPOSITO\b|\bPELO\b|\bDE\b/g, " "));
}
// ── Classificador ────────────────────────────────────────────────────────────
function classifyIncome(tx, ctx) {
    ...
    if (tx.internal === true || (0, internal_transfer_1.isInternalTransfer)(tx.description)) {
