...
const SORTED_KEYWORDS = Object.entries(CATEGORY_MAP)
...
function categorizeTransaction(description, type) {
    ...
    if ((0, internal_transfer_1.isInternalTransfer)(description))
        return "Transferencias";
    // Regra 1: entradas → Recebimentos
    if (type === "income")
        return "Recebimentos";
    const norm = normalizeDesc(description);
    if (FORCE_OUTROS.some((p) => p.test(norm)))
        return "Outros";
    ...
    if (norm.includes("CREDITO")) {
        if (/EMPRESTIMO|FINANCIAMENTO|AFINZ/.test(norm))
            return "Emprestimos";
        return "Recebimentos";
    }
    return "Outros";
...
//# sourceMappingURL=categorize-keyword.js.map